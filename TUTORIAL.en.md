...
# VibeSKU Clips User Tutorial (beginner edition — every step spelled out)
...
Still stuck afterwards → [open an Issue](https://github.com/UllrAI/VibeSKU-Clips/issues) or [start a Discussion](https://github.com/UllrAI/VibeSKU-Clips/discussions). English or Chinese, both fine.
...
3. [Step 1 — Install VibeSKU Clips (pick one of three)](#3-step-1--install-vibesku-clips-pick-one-of-three)
...
| Term | In plain words |
|---|---|
| **Key / API key** | A secret string (like `sk-xxxx`) you get after signing up with an AI platform. **You pay that platform directly — VibeSKU Clips never takes a cut and never handles your money.** |
| **BYOK** | Bring Your Own Key. VibeSKU Clips is free and open source; AI usage is billed by the platform you chose. |
...
| **TTS** | Text-to-speech, i.e. automatic voice-over. VibeSKU Clips defaults to **free Microsoft Edge TTS** — no key. |
...
## 3. Step 1 — Install VibeSKU Clips (pick one of three)
...
Open 👉 **https://github.com/UllrAI/VibeSKU-Clips/releases/latest**

Scroll to the **Assets** section and grab the file for your OS:

| OS | File | Notes |
|---|---|---|
| macOS (Apple Silicon: M1/M2/M3/M4) | `VibeSKU Clips-x.x.x-arm64.dmg` | |
| Windows 10/11 (64-bit) | `VibeSKU Clips.Setup.x.x.x.exe` | Installer |
| Linux (64-bit) | `VibeSKU Clips-x.x.x.AppImage` | Portable executable (shipped from v0.8.90 onward; older releases have none — use Docker or run from source) |
...
1. Open the `.dmg` and drag VibeSKU Clips into Applications;
2. In Applications, **right-click the VibeSKU Clips icon → Open** (right-click matters; a plain double-click won't offer the bypass);
...
> ⚠️ The Windows and Linux builds are produced automatically and are not hand-tested each release — [issue reports](https://github.com/UllrAI/VibeSKU-Clips/issues) welcome.
...
| `-v vibesku-clips-data:/data` | **The data volume — don't skip it.** Projects, product images and rendered videos live here; without it, deleting the container deletes everything |
| `ghcr.io/ullrai/vibesku-clips:latest` | Official image, rebuilt and smoke-tested on every release |
...
docker pull ghcr.io/ullrai/vibesku-clips:latest   # upgrade: pull, stop the old container, re-run the same command — keep the same -v and your data survives
```
...
**Why it's required:** VibeSKU Clips ships no model of its own, so writing the spoken script calls out to an AI platform. **This is the only mandatory setup** — footage, voice-over and composition are free and keyless.
...
**VibeSKU Clips itself is free and open source forever.** AI usage is billed **per second, directly by the model platform you chose** — no markup, no revenue share, we never touch your money.
...
**Prerequisite:** a running VibeSKU Clips instance (`pnpm dev` / `pnpm start` / Docker).
...
```bash
node bin/vibesku-clips.mjs trends                                        # trending topics (--geo US uses Google Trends)
node bin/vibesku-clips.mjs create --topic "pour-over coffee at home" --quality hd --bgm   # one sentence → video, prints videoUrl
node bin/vibesku-clips.mjs list                                          # list projects
node bin/vibesku-clips.mjs get --project <id>                            # latest rendered video URL
node bin/vibesku-clips.mjs qc --project <id>                             # QC (black frames / silence / loudness)
node bin/vibesku-clips.mjs gate --project <id> --strict                  # release gate, exit code 2 when it blocks
node bin/vibesku-clips.mjs transcript --project <id> --media <mediaId>   # inspect footage transcript and edit revision
node bin/vibesku-clips.mjs transcript-edit --project <id> --media <mediaId> --plan edit.json --revision 0 --operation edit-001  # dry-run first; add --apply after confirmation
node bin/vibesku-clips.mjs timeline --project <id> --media <mediaId> --plan edit.json --format otio --out edit.otio  # export an editable professional timeline
node bin/vibesku-clips.mjs --help                                        # everything else
```
...
```bash
claude mcp add vibesku-clips -- node /absolute/path/vibesku-clips/mcp/vibesku-clips-mcp.mjs
```

Then just say "make a vertical product video from this link with VibeSKU Clips". Full tool list: [mcp/README.md](mcp/README.md).
...
| Symptom | Cause | Fix |
|---|---|---|
| macOS "developer cannot be verified" / "damaged" | App is unsigned | Right-click → Open; or `xattr -cr /Applications/VibeSKU Clips.app` |
| Windows "protected your PC" | SmartScreen | More info → Run anyway |
| Linux AppImage does nothing | Not executable | `chmod +x VibeSKU Clips-*.AppImage` |
...
| Symptom | Cause | Fix |
|---|---|---|
| Projects vanished after a restart | No data volume | Always include `-v vibesku-clips-data:/data` |
...
|---|---|
| macOS desktop | `~/Library/Application Support/VibeSKU Clips/data` |
| Windows desktop | `%APPDATA%\VibeSKU Clips\data` |
| Linux desktop | `~/.config/VibeSKU Clips/data` |
| From source | `data/` inside the project folder |
| Docker | Volume `vibesku-clips-data` (`/data` inside the container) |
...
- **Uninstall:** delete the app (and that data directory if you want it gone); for Docker, `docker rm` the container and `docker volume rm vibesku-clips-data`.
...
Open an [Issue](https://github.com/UllrAI/VibeSKU-Clips/issues) or [Discussion](https://github.com/UllrAI/VibeSKU-Clips/discussions) (**English or Chinese**) with these three things:
