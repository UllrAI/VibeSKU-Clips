...
VibeSKU Clips 把商品资料转化为一条时长可选的本地化 UGC 视频。运营提交商品链接或
...
- **三类内容模板。** 人物讲解、场景种草、使用介绍，每类都有固定的节拍结构与可轮换
  的开场角度。
- **人物参考。** 上传获授权的照片，或描述虚构成年形象；确认后的图片用于引导每段
  镜头的人物与商品外观。
- **一次做好一条。** 商品、模特、脚本、可选分镜与成片是一条引导路径，每个高成本步骤
  都先由人确认。
- **基于实测的质检。** FFprobe 检查媒体时长和音视频轨，云端 ASR 核对口播并提供
  字幕时间；商品外观与人物一致性由运营审核。
- **审核与再生成。** 可以只重做一个镜头并复用其他镜头，保留已有成片与版本。
- **导出附交付清单。** 每条素材包含编号、商品、语言、市场、模特、授权说明与合规声明。
- **资产与消耗记录。** 商品、模特、脚本与合格成片保留来源、授权与版本关系；解析、
  脚本、生成、重试与再生成分别计量。

成片规格：可选 15–120 秒、9:16 或 16:9，并按供应商能力选择分辨率；随附封面、字幕、发布配文与合成内容披露说明。

## 🧱 生产流程

| 阶段 | 任务                  | 说明                                                |
| :--- | :-------------------- | :-------------------------------------------------- |
| 录入 | `ugc.product.ingest`  | 通过 Firecrawl 导入商品页并提取事实，等待确认或补充 |
| 脚本 | `ugc.work.script`     | 根据确认的商品、模特、语言和市场写一份脚本          |
| 分镜 | `ugc.work.storyboard` | 每一拍画一张关键帧，画完停下等待确认                |
| 视频 | `ugc.work.video`      | 创建独立镜头任务与不可变的合成版本                  |
| 镜头 | `ugc.work.segment`    | 逐段生成和归档，用云端 ASR/TTS 核对语音             |
| 合成 | `ugc.work.compose`    | 使用 FFmpeg 探测、统一规格、加字幕并拼接成片        |
...
还可选择 Seedance 2.0 或 2.5，并按模型限制可用分辨率。普通 Worker 负责供应商
任务，独立的 render worker 使用 FFmpeg/FFprobe 合成；生成文件流式归档到私有 R2。
