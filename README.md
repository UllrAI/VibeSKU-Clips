...
> 🎬 **图生视频是质量主路**：配了视频模型后，出图会**自动「图生视频」把每个分镜转成真动态镜头**（i2v，商品图当首帧不失真），替掉「静图 + 假运镜」，质感大幅提升。**运镜提示词引擎**把脚本里的镜头语言（推拉摇移）+ 分镜类型专属运镜 + 商品保真约束（logo/文字不扭曲）+ 稳定性约束组装成 i2v 专用提示词——实测对比：无约束提示词会凭空长出手、商品被挤出画面，运镜提示词全程锁商品（A/B 真实调用验证）。**批量生成走链式首尾帧**（即梦同款玩法）：上一镜的尾帧钉在下一镜关键帧上，转场由模型在镜头内生成，成片拼接处画面连续不跳切（实测 5 秒内完成「大理石台面→客厅木桌」无缝场景过渡）；合成器对略超槽位的 AI 片段**限幅变速压入**（≤1.35x）保住首尾两端，不再裁尾断链。可一键开关、失败自动回退静图（回退运镜也改 ease-out 缓出，减速到位，不再匀速廉价感）；不配视频模型则走 0 成本免 Key 拼接兜底，配素材时**多候选按竖屏+相关度+去重择优**。
>
> 🎛 **运镜强度与逐镜兜底**（v0.8.61）：**运镜强度三档**（轻/中/强，素材页一键切换、全局记忆），同一关键帧可出「缓慢克制」或「大胆利落」两种气质；**逐镜重跑动态**——对某个动态镜头不满意时，保持关键帧画面不变只重新生成运动（不推翻全片、不重出图），i2v 视频落库时**自动保留来源关键帧**，重跑后链式首尾帧照常接力；**i2v 提示词工程包**（Seedance 官方指南落地）：每镜显式声明「连续单镜头拍摄」防模型中途切镜、音效行锁定「自然环境音、无人声」防无配音分镜冒出乱造说话声、脚本运镜指令**冲突消解**（「固定镜头」+「环绕」这类自相矛盾的指令自动回退到分镜类型默认运镜，杜绝画面犹豫抖动）。
>
...
>
> 📐 **生图尺寸自动适配**（v0.8.57，修复 [#18](https://github.com/xixihhhh/clipforge/issues/18)）：Atlas Cloud 各图像模型的尺寸协议互不相同（GPT Image 2 要求宽高被 16 整除、Seedream 只收 `宽*高` 预设、Nano Banana 用比例+档位），现按模型**自动映射到合法尺寸且保持比例精确不失真**（9:16 → 1152x2048），不会再出现「尺寸不合法但已扣费」的失败任务；三条模型链路均已真实调用验证。
...
- [x] **片尾扫码购买**（把二维码烧进成片**片尾最后几秒**引导转化，后处理成片不改合成管线；居中落在干净中带、避开视频自带贴片；**站外导流平台把关**：`platform=douyin` 默认拒绝（2026-07 抖音处罚，`force` 可强制）、其他国内平台附风险提示；`POST /api/project/[id]/end-card` / CLI `clipforge endcard [--force]` / MCP `clipforge_end_card`）
