...
| 🧠 **生产控制台** | 导演模式统一看工作流、真实模型费用区间、预计耗时、智能模型推荐、版本树、失败诊断与项目视觉记忆 |
| 🔍 **媒体解构 → 项目记忆** | 任意图片/视频识别主体、光线、构图、镜头与节奏，反推可用提示词并保存到具体项目继续创作 |
| 🧬 **一致性 + QC 修复闭环** | 人物/商品/服装锚点进入真实生图与视频请求；成片质检自动拆成免费修复或需确认的单阶段重做计划 |
