...
| 🧭 **一条路，按需展开** | 默认只问内容与出片方式；分镜、模型、成本、版本与修复都在原地折叠，随时展开 |
...
- 🧭 **一个创作入口**：侧边栏只留「创作 / 项目」两个主入口，热点复刻、视觉拆解、批量、SKU 库、主播库收进折叠的工具箱；创作页默认只问内容与出片方式，出镜形式、主播、生产档位和高级新建都在原地折叠展开（issue #1）。
...
- 🔌 **单一媒体网关 + 设置重排（v0.9.2）**：①图片与视频全部改走 **Prism** 一个网关，Atlas / fal.ai / Replicate / 火山 / 阿里 / 硅基 / OpenAI 七套适配器与各自的 Key、模型 id 词表一并删除；默认视频模型 **MiniMax H3**（音画同出）、默认图片模型 **GPT Image 2**（quality 默认 low）。②每个模型的合法时长、比例、分辨率与参考素材上限写成本地目录，下单前先对齐，不再「等四分钟才被 422 拒绝且已计费」。③脚本模型改用 **Vercel AI SDK** 走 OpenAI 兼容协议，预设以 **OpenRouter** 打头。④设置页从七个平级页签收敛成 连接 / 生成 / 配音 / 高级 四段，首屏只有一个明确的连接动作，并把「软件免费 / 免费素材链免 Key / 脚本模型要 Key / Prism 按量计费」四件事分开写清。⑤`/project/topic` 这个无人可达的第三创建入口删除，`/project/new` 降为创作页折叠区里的「高级新建」。
...
想更高画质再加 Key：图片与视频统一走 **[Prism](https://prism.ullrai.com)** 一个网关，一对 Key/Secret 覆盖 **MiniMax H3**（音画同出，默认）、**Seedance 2.5**、Sora 2、万相 2.6 与 **GPT Image 2** 等全部模型；写脚本用任意 OpenAI 兼容端点（推荐 OpenRouter）。自部署开源（AGPL-3.0），数据全在本机不上云。
...
| 创作页·免费/AI 双档明选 | 脚本闸门·一次点击才花钱 | 高级新建 |
|:---:|:---:|:---:|
...
全部生图与生视频统一走 **[Prism](https://prism.ullrai.com)** 一个网关：一对 API Key / Secret，上游多家厂商由 Prism 自己竞速与兜底，本地不再维护七套 SDK、七种模型 id 和七个 Key。

| 类型 | 模型 | 说明 |
|------|------|------|
| **视频** ⭐默认 | **MiniMax H3** | 音画同出——口播、环境声、动作一次生成，单镜不用再另配音；480p/720p，1–15s，支持首尾帧与参考图 |
| 视频 | MiniMax H3 Max | H3 高配档，画面更稳，仅 16:9 |
| 视频 | **Seedance 2.5** | 单镜最长 30s，参考图 30 张 / 参考视频 10 条，做复刻与多主体最合适 |
| 视频 | Seedance 2.0 / Fast / Mini | 多模态参考最全，支持参考视频与视频续写；Fast 更快、Mini 最省 |
| 视频 | Sora 2 / Sora 2 Pro | 图片作风格参考（不是严格首帧），10–25s |
| 视频 | 万相 2.6 / 2.6 Flash | 图片作严格首帧，5/10/15s |
| **图片** ⭐默认 | **GPT Image 2**（quality 默认 low） | 商品还原准，质量档位可调；草稿阶段保持 low 最省 |
| 图片 | GPT Image 2 VIP / 1.5 / 1 | 高优先级通道 / 上一代 / 最早一代兜底 |
| 图片 | Nano Banana / Pro / 2 / 2 Lite | 出图快，适合大批量草稿；Pro 支持 4K |

> 每个模型能接受的时长、比例、分辨率、参考素材上限都写在本地目录里（`src/lib/providers/prism-catalog.ts`，逐条来自线上接口的真实报错），下单前先在本地对齐——不会出现「等了四分钟才被 422 拒绝且已计费」。

> **LLM（脚本生成）** 走 OpenAI 兼容协议，通过 Vercel AI SDK 调用，内置 **OpenRouter**(一个 Key 聚合 400+ 模型) / OpenAI / DeepSeek / Kimi / 智谱 / MiniMax / 豆包 一键预设，外加 **Ollama 本地**（离线免 Key）——任意 OpenAI 兼容端点都能填。
...
1. 打开 **设置 → 连接**，填 **脚本模型 Key**（写脚本必需，推荐 [OpenRouter](https://openrouter.ai/keys)，一个 Key 聚合 400+ 模型）
2. 想让 AI 出画面，再填 **[Prism](https://prism.ullrai.com)** 的 API Key 与 Secret（生图和生视频共用这一对）；只用免费素材可以不填
3. **生成** 里已经预设好默认值（视频 MiniMax H3、图片 GPT Image 2 + quality low），不满意再改
4. （可选）**高级** 里添加出镜人物、配置品牌视觉
5. 回到 **创作**，丢一张商品图或写一句主题，直接开始

> 费用说明：软件本身免费开源；免费素材链不要 Key；脚本模型按各家价格计费；Prism 按次计费。三者互不相干。
...
| **测试** | Vitest（1130+ 用例）+ Playwright (E2E) |
...
src/
├── app/                              # 页面路由
│   ├── page.tsx                      # 首页（重定向到 /start）
│   ├── start/                        # 创作页（唯一创建入口：图片 / 链接 / 一句话）
│   ├── products/                     # 商品库管理
│   ├── batch/                        # 批量出片
│   ├── settings/                     # 设置（连接 / 生成 / 配音 / 高级 四段）
│   ├── project/
│   │   ├── new/                      # 高级新建（品类 / 时长 / 脚本风格 / 广告模板）
│   │   ├── clone/                    # 爆款复刻
│   │   └── [id]/
...
│
├── lib/
│   ├── providers/                    # Prism 媒体网关 + 多源免费素材引擎
│   │   ├── prism.ts prism-catalog.ts #   唯一生图/生视频通道 + 模型约束目录
...
图片与视频统一走 **Prism** 网关，一对 Key/Secret 覆盖下面全部模型；每个模型的合法时长、比例、分辨率和参考素材上限见 `src/lib/providers/prism-catalog.ts`（逐条来自线上接口的真实报错，本地先对齐再下单）。

### 视频生成

| 模型 | 音频 | 时长 | 分辨率 | 特点 |
|------|------|------|--------|------|
| **MiniMax H3** ⭐默认 | 原生（不可关） | 1–15s | 480p / 720p | 音画同出，口播与环境声一次生成；支持首尾帧对与 ≤9 张参考图、≤3 条参考音频 |
| **MiniMax H3 Max** | 原生（不可关） | 5–15s | 480p / 720p | H3 高配档，画面更稳，仅 16:9 |
| **Seedance 2.5** | 开关可控 | 4–30s | 480p–1080p | 单镜最长 30s，参考图 30 张 / 参考视频 10 条 / 参考音频 10 条，复刻与多主体首选 |
| **Seedance 2.0** | 开关可控 | 4–15s | 480p–1080p | 多模态参考最全，支持参考视频与视频续写 |
| **Seedance 2.0 Fast / Mini** | 开关可控 | 4–15s | 480p / 720p | Fast 更快，Mini 成本最低 |
| **Sora 2 / Sora 2 Pro** | 原生（不可关） | 10–15s / 15–25s | 720p / 1080p | 图片作风格参考，不是严格首帧 |
| **万相 2.6 / 2.6 Flash** | 开关可控 | 5 / 10 / 15s | 480p–1080p | 图片作严格首帧 |

### 图片生成

| 模型 | 质量档 | 特点 |
|------|:---:|------|
| **GPT Image 2** ⭐默认 | ✅ | 商品还原准，质量档位可调（默认 `low`，草稿阶段最省）|
| **GPT Image 2 VIP** | ✅ | 同上，走高优先级通道 |
| **GPT Image 1.5 / 1** | ✅ | 上一代 / 最早一代，成本更低，可兜底 |
| **Nano Banana / Pro / 2 / 2 Lite** | - | 出图快，适合大批量草稿；Pro 支持 4K，Lite 仅 1K 最省 |

> 带货场景建议对商品原图做**图生图重绘**（传 `reference_urls`），锁定商品主体不被篡改。
> 视频模型的原生音频与外接 TTS 二选一：H3 这类音画同出的模型开着原生音频就不必再叠配音。
