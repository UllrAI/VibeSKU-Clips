...
| 🧬 **跨镜头一致** | 人物、商品、服装与场景锚点进入真实生成请求，商品不乱变 |
...
- ✂️ **按文字剪视频（v0.8.95）**：导入 MP4 / MOV / WebM / MKV / M4V，音频在浏览器本地转写为逐词时间轴；点掉一句话就删除对应画面，也可自动删静音、烧录逐词字幕。原片始终保留，每次输出都是独立版本并自动进入成片、导出与 QC 链路；优先 WebGPU，无法使用时自动切换兼容模式。
