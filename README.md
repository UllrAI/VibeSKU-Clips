...
想更高画质再加 Key：图片与视频统一走 **[Prism](https://prism.ullrai.com)** 一个网关，一对 Key/Secret 覆盖 **MiniMax H3**（音画同出，默认）、**Seedance 2.5**、万相 2.6 与 **GPT Image 2** 等全部模型；写脚本用任意 OpenAI 兼容端点（推荐 OpenRouter）。自部署开源（AGPL-3.0），数据全在本机不上云。
