...
本项目建设集**商品理解、创意策划、视频生成、智能剪辑与质量审核**于一体的 AI UGC 视频智能生产平台，为 TikTok 账号矩阵持续提供适配不同语言与市场的商品短视频，支撑内容发布、商品挂车与自然流量获取。
...
平台按多语言、多区域能力设计，覆盖中文、英语、日语、韩语、葡萄牙语、西班牙语等内容生产方向，适配北美、拉美、欧洲及日韩等市场的表达需求。可结合美国、墨西哥、巴西、日本等目标市场配置语言、人物风格与内容场景，支持同一商品形成面向不同受众的本地化版本。
...
平台交付差异化成片与商品对应清单，运营通过既有工具完成账号发布、挂车和效果观察。账号标签仅用于内容分组与导出，无需接入账号登录或发布权限。

| 矩阵使用要求               | 视频生产侧的对应能力                                             |
| -------------------------- | ---------------------------------------------------------------- |
| 不同账号有不同受众与风格   | 可复用的内容配置：目标人群、模特、语气、场景；批量任务可按组应用 |
| 同一商品需要多条可发布素材 | 多脚本、多视觉方案，提供相似内容提示和人工对比选用               |
| 视频与挂车商品必须一致     | 成片关联商品、变体及目标市场商品链接，随素材包交付               |
| 大量素材需要准确分发       | 按商品或账号标签分组导出，附文件与商品对应清单                   |
...
将输出语言与目标市场独立配置，使同一语言也能适配不同地区的表达习惯。商品资料可使用原始语言输入，系统结合当地商品信息生成目标语言文案、配音与字幕。
...
多语言版本分别完成口播调时与字幕同步，保持 15 秒成片节奏。区域化表达由对应语言样片验证，商品价格、规格和承诺以目标市场资料为准。
...
批量能力覆盖“多个商品”和“一个商品多个视频”，支持按商品设置目标语言、市场、脚本数量、每个脚本的视频数量及模特。共用 Brief 可批量应用，单个商品可单独调整。
...
| **同商品、多语言版本**     | 复用商品与创意方向，按目标语言和市场分别生成脚本与视频             | 支持跨市场素材准备与创意延展               |
| **优选方案继续生产**       | 复用已选中的商品、脚本和模特设置，追加视频版本                     | 延续满意的表达方式                         |

同脚本多视频适合筛选表演与画面；用于矩阵分发时，优先从开场、叙述角度和场景形成实质差异，避免将近似版本直接批量分发。同脚本指保持既定口播与内容结构，画面可以变化；口播超出 15 秒承载范围时提示精简，不擅自改变锁定脚本。跨商品或跨语言复用的是脚本结构与风格，商品事实和具体台词分别适配；翻译或本地化后保存为独立脚本版本。

提交前展示批次总视频数与预计消耗，按明确配置执行，避免无意扩大组合。每条视频均能追溯所属商品、脚本与模特，批次内单项失败可独立处理。

### 4.6 成片评审与定向再生成

生成结果按商品、语言、脚本和视频版本分组展示，可附账号标签。用户可预览对比、查看脚本和商品对应关系，标记选用、待定或不采用，并分组下载选中视频。平台提示批次内及已导出素材中的重复或高度相似内容，辅助筛选。
...
| 工作区域       | 用户操作                                            | 系统承担                                 |
| -------------- | --------------------------------------------------- | ---------------------------------------- |
| **创建任务**   | 提供商品与 Brief，选择语言/市场、可选模特及生产数量 | 输入检查、配置汇总与预计消耗             |
| **生产进度**   | 查看单条或批次进度，补充必要信息、取消未执行任务    | 自动解析、编写脚本、生成、质检及异常处理 |
| **成片工作台** | 预览比较、核对商品、选用、分组下载或重新生成        | 相似内容提示、商品清单、版本与导出记录   |
...
- **跨市场素材准备**：同一商品面向美国、墨西哥、巴西分别制作英语、西班牙语和葡萄牙语版本，每个市场 2 条，共 6 条；商品链接和表达分别匹配。
...
该结构作为创作参考，节奏可随脚本调整。系统控制口播长度、字幕同步与总时长，避免生硬加速或截断对白。以开场吸引停留，以清晰展示建立商品认知，结尾引导查看站内商品；字幕和重点信息避开按钮及挂车展示区域，不在画面中伪造可点击购物组件。
...
| 配套资产   | 封面、字幕、目标语言发布配文、必要披露说明及商品对应清单 |
| 使用方式   | 按商品或账号标签分组导出，支持选用、重新生成和版本追溯   |

### 挂车素材交付

每条选中视频随附商品名称、变体、目标市场商品链接及素材编号。输入链接若来自其他站点或仅用于参考，目标挂车链接由用户补充并核对；未补齐时标记“待匹配商品”，不自动视为可挂车素材。

运营发布前核对实际挂车商品、账号资格和商品可售状态。价格、优惠和库存具有时效性，常规视频优先采用可持续使用的卖点；需要促销信息时记录条件与有效期，过期素材提示复核。
...
| **选用前：内容可用** | 目标语言自然度、商品对应、内容差异与整体观感 | 用户集中审核、核对商品清单、比较与选用         |
...
| 试运行验收 | 验证多商品、同/异脚本、多语言、矩阵分组交付及商品清单，开展培训 | 验收报告、操作文档、培训与交接       |
...
| **批量生产能力**   | 多商品、同/异脚本、多模特、多语言、内容分组及选择性追加                        |
| **矩阵素材交付**   | 相似内容提示、分组导出、商品对应清单、目标语言配文与导出记录                   |
...
| 多语言适配 | 约定语种与区域的口播、字幕及文案一致，表达自然、时长合格，各版本商品信息正确 |
| 矩阵交付   | 按商品或账号标签准确导出，视频与商品清单一一对应，未匹配商品及过期信息有提示 |
| 生产效率   | 对比现有制作方式，评估人工处理时间与成片周期                                 |
| 成本可控   | 按合格片统计实际生成、重试和处理成本                                         |

建议以**最终任务合格率达到 85%、经常性人工处理时间降低 50%**作为试点优化目标。具体阈值、样本和计算口径在样片验证后确认；上述数值为建设目标，非既有实测结果。
