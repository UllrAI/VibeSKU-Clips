  ...
  return (
    <div className="mb-6 rounded-xl border border-border/60 bg-muted/10 px-3 py-3 sm:px-4" aria-label={t("preflightTitle")}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
            ...
            {t("preflightTitle")}
            <span className={`rounded-full px-2 py-0.5 text-[10px] ${caps.confidence === "known" ? "bg-success/12 text-success" : "bg-warning/12 text-warning"}`}>
      ...
      {result.adjustments.length === 0 && result.warnings.length === 0 ? (
        <p className="mt-3 flex items-center gap-1.5 text-xs text-success"><LuCheck className="h-3.5 w-3.5" aria-hidden="true" />{t("preflightReady")}</p>
      ) : (
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {result.adjustments.map((item) => (
            <div key={`${item.field}-${item.code}`} className="rounded-lg border border-warning/20 bg-warning/8 px-3 py-2 text-xs">
              <div className="flex items-center gap-1.5 font-medium text-warning"><LuTriangleAlert className="h-3.5 w-3.5" aria-hidden="true" />{t(`preflightField_${item.field}`)}</div>
          ...
          {result.warnings.map((warning) => (
            <div key={warning} className="rounded-lg border border-warning/20 bg-warning/8 px-3 py-2 text-xs text-warning">
