 ...
 * first consumer is the infinite-canvas workbench running the VibeSKU Clips video
 * node plugin (canvas at :3800/:3000 → VibeSKU Clips at :3457). Without these
 ...
 * via VIBESKU_CLIPS_CORS_ORIGINS (comma-separated full origins).
 */
...
function allowedOrigin(origin: string | null): string | null {
  ...
  const extra = (process.env.VIBESKU_CLIPS_CORS_ORIGINS || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
