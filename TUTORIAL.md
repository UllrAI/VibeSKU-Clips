...
8. [进阶二：四步工作台逐页说明](#8-进阶二四步工作台逐页说明)
...
| **渐进披露** | 只有一条创作主路，默认只问「内容」和「出片方式」。分镜、导演台、生产档位这些专业控件都在原地折叠着，点开就有，不用切模式。 |
...
### 4.1 路线 A（最省事）：OpenRouter 一个 Key 通吃

一个 Key 就能调 400+ 个模型，以后想换模型只改一格模型名。

1. 打开 https://openrouter.ai/keys 注册，充值几块钱，创建一个 Key 并复制（**Key 只显示一次，先存下来**）；
2. VibeSKU Clips → **设置 → 「连接」**，在「脚本模型服务」卡片的快捷预设里点一下 **OpenRouter** —— baseUrl 和模型名会自动填好；
3. 在 **API Key** 那一格粘贴刚复制的 Key；
4. 点 **「测试连接」**，看到 **连接正常** 就完事了（设置**改了即存**，不用点保存）。

> 也可以直接在创作页点「创建项目并生成脚本」——没配 Key 时会就地弹出连接面板，填完接着跑，不用来回跳页。

---

### 4.2 路线 B（最便宜）：DeepSeek，手动填三格

1. 打开 https://platform.deepseek.com 注册，充值几块钱，在「API Keys」页面创建一个 Key 并复制；
2. VibeSKU Clips → **设置 → 「连接」**；
...
3. VibeSKU Clips → 设置 → 连接 → 快捷预设点 **「Ollama 本地」**；
4. 模型名要写全（含 tag），比如 `qwen2.5:7b-instruct`；不确定就点 **「读取可用模型」** 看本机装了哪些；
5. 点「测试连接」→ 成功即可。

> 建议 7B 及以上的 instruct 模型。0.5B/1.5B 写不出结构化脚本，会被拦下并提示换模型。

---

### 4.4 其它平台预设（点一下自动填 baseUrl 和模型）

设置 → 连接 → 快捷预设里内置了这些，点一下只需再补 Key：

| 预设 | baseUrl | 默认模型 | 备注 |
|---|---|---|---|
| OpenRouter | `https://openrouter.ai/api/v1` | `openai/gpt-5.4` | 推荐，一个 Key 聚合 400+ 模型 |
...
> 任何兼容 OpenAI 协议的平台都能用：自己填 baseUrl + Key + 模型名即可。
...
| 超时 | 网络到不了该平台 | 换平台，或在 baseUrl 里填你的代理地址 |

> 连接测试走的是**服务端**，不受浏览器跨域限制；失败基本都是 baseUrl / Key / 模型名三者之一写错。

---

### 4.6 想让 AI 直接画画面？再填一对 Prism 的 Key

只用免费素材可以跳过这一节。要 AI 生图、生视频，就到 **设置 → 连接 → 「图片与视频 · Prism」**：

1. 打开 https://prism.ullrai.com 注册，拿到 **API Key** 和 **API Secret**（两个都要，缺一个不行）；
2. 两格分别粘进去，点 **「测试连接」**——这个测试只查一个不存在的任务，**不生成任何东西、不产生费用**；
3. 生成用的模型在 **设置 → 生成** 里，默认已经选好：视频 **MiniMax H3**（音画同出，口播和环境声一次生成）、图片 **GPT Image 2**（质量档默认「低」，草稿阶段最省）。

> 一对 Key/Secret 覆盖全部生图与生视频模型，不用再逐个平台配。费用按次计到你自己的 Prism 账户，VibeSKU Clips 本身不收钱。
...
- 想自己精修每个镜头 → 点 **「逐镜精修 →」**（见[第 8 节](#8-进阶二四步工作台逐页说明)）。
...
| 环节 | 实测参考价 |
|---|---|
| 九宫格分镜（一次生图，把所有镜头画齐） | 约 ¥1.3 |
| 一键整片（Seedance 2.5，12 秒） | 约 ¥26 |
| 便宜档（Seedance Mini，约 ¥0.28/秒，8 秒） | 约 ¥2.2 |

> 价格随平台调整，**以你所选平台的实时价为准**；费用在选项上直接标明，点之前就知道要花多少。

### 7.2 要多配什么

免费档只要 LLM Key；AI 档还要**生图模型**和**视频模型**：

**设置 → 连接 → 「图片与视频 · Prism」** 填好 API Key 和 API Secret 就行（见[第 4.6 节](#46-想让-ai-直接画画面再填一对-prism-的-key)）。模型在 **设置 → 生成** 里，默认已经选好，不改也能用。
...
- 任何一步失败，都可以手动逐步完成，或退回免费快剪出片。

---

## 8. 进阶二：四步工作台逐页说明

创建项目后，顶部会出现四步进度条：**脚本 → 素材 → 视频 → 导出**。每一页的高级控件都折叠在原地，需要时点开即可，不用切换任何模式。
...
| **批量出片** | 侧边栏 → 工具箱 → 批量出片 | 勾选多个商品 + 统一配置（视频模式/脚本风格/品类）→ 一键排队逐条产出，大促前一晚跑十条 |
...
| 「还没连上 Prism」 | AI 成片缺画面模型 | 设置 → 连接 → 填 Prism 的 API Key + API Secret |
| 生成的画面报 422 | 参数超出该模型允许范围 | 报错里会写清可选值；设置 → 生成 里按提示改时长/比例/分辨率 |
...
| 自动成片失败 | 中间某步失败 | 点「转手动编辑」逐步完成；或退回免费快剪 |
