...
`vibesku-cli` is the first-party command-line interface for VibeSKU Clips.
...
- stores CLI tokens in `~/.vibesku-cli/config.json`
- supports API key auth through `VIBESKU_CLI_API_KEY`
...
pnpm vibesku-cli -- auth refresh --base-url http://localhost:3000
pnpm vibesku-cli -- auth logout
```
